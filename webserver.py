...
# curl http://127.0.0.1:8080/test.html --header "If-Modified-Since: Wed, 22 Oct 2025 16:00:00 GMT"
...
# curl http://127.0.0.1:8080/teehee.txt

# 404 
# curl http://127.0.0.1:8080/doesnotexist.html
# 505 
# curl "http://127.0.0.1:8080/test.html?v=2"
...
def handle_client(connection):
    ...
    if version != "HTTP/1.1":
        connection.sendall(build_response(505, b"HTTP Version Not Supported"))
        return

    file_path = os.path.join(WEB_ROOT, path.lstrip("/"))

    if not os.path.exists(file_path):
        connection.sendall(build_response(404, b"Not Found"))
        return

    if not os.access(file_path, os.R_OK):
        connection.sendall(build_response(403, b"Forbiden"))
        return
